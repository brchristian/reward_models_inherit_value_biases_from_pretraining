...
def main():
    ...
    # Use only models whose CSVs exist
    available = [m for m in FIGURE_3_MODELS_ALL
                 if os.path.exists(os.path.join(logprobs_dir, f"{safe_name(m)}.csv"))]
    skipped = set(FIGURE_3_MODELS_ALL) - set(available)
    if skipped:
        print(f"Skipping (no CSV): {', '.join(skipped)}")

    print("Loading per-model logprob CSVs...")
    merged_df = load_model_logprobs(logprobs_dir, available, PROMPT_KEY)

    print(f"Merged on {len(merged_df)} common tokens across {len(available)} models")

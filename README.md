...
scripts/
├── download_data.py            Download pre-computed logprob CSVs from HuggingFace
├── fetch_logprobs.py           Shared helper: auto-download missing CSVs
...
Scripts that need log-probability data will automatically download the required CSVs (~2 GB total) from [HuggingFace Hub](https://huggingface.co/datasets/Oxford-HIPlab/iclr2026-lm-logprobs) on first run. No GPU required.
...
### Log-probabilities (Figures 2, 3, and Table 1)

Figure and table scripts automatically download the CSVs they need from [HuggingFace Hub](https://huggingface.co/datasets/Oxford-HIPlab/iclr2026-lm-logprobs) on first run. To download all 12 models upfront, or to regenerate from scratch with a CUDA GPU:

```bash
python scripts/download_data.py                              # download all 12 models
python scripts/generate_logprobs.py                          # regenerate (requires GPU)
```
